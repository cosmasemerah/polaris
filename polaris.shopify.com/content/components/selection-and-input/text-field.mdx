...
Reference the [text field pattern](https://polaris.shopify.com/patterns-legacy/text-fields) page for additional text field content guidelines.

---

## Related components

- To lay out the elements in a responsive form, use the [form layout component](https://polaris.shopify.com/components/selection-and-input/form-layout)
- To indicate that a group of interdependent form inputs is invalid, use the [inline error component](https://polaris.shopify.com/components/selection-and-input/inline-error)
...
The `label` prop is required to convey the purpose of the input to all merchants.
...
Use the `placeholder` prop to provide additional instructions. However, don’t rely on placeholders alone since the content isn’t always conveyed to merchants using assistive technologies.
...
Although you can use the `autoFocus` prop to automatically move focus to the text field, it’s generally best to avoid focusing on fields automatically. The `autoFocus` prop is set to `false` by default. Do not use `autoFocus` in cases where forcing focus would skip other controls or content of equal or greater importance. Only focus text field automatically if the primary task is to filter or search, such as in a searchable listbox or index filters.
